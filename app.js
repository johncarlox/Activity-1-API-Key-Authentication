...
const floatSummary     = document.getElementById("floating-summary");
const floatDist        = document.getElementById("float-dist");
...
/* ── map update ───────────────────────────────────────── */
function updateMap(start, destination, onComplete) {
  // Dynamically get the frame width to avoid cropping
  const frameWidth = document.querySelector('.map-frame')?.offsetWidth || 1000;
  ...
  /* Hide map and show placeholder while loading with transition */
  routeMap.classList.add('map-hidden');
  floatSummary.classList.add('map-hidden');
  mapPlaceholder.classList.remove('map-hidden');
  mapPlaceholder.classList.add('loading-state');

  /* Update labels text early */
  if (mapLabels) {
    labelStartText.textContent = start;
    labelEndText.textContent   = destination;
    mapLabels.hidden = true;
  }

  /* Listen for image load to trigger animation */
  routeMap.onload = () => {
    mapPlaceholder.classList.add('map-hidden');
    mapPlaceholder.classList.remove('loading-state');
    routeMap.classList.remove('map-hidden');
    floatSummary.classList.remove('map-hidden');
    if (mapLabels) mapLabels.hidden = false;
...
async function handleSubmit(event) {
  ...
  try {
    ...
    /* Update floating summary values */
    floatDist.textContent = `${Number(route.distance ?? 0).toFixed(2)} mi`;
    floatTime.textContent = route.formattedTime || secondsToClock(route.realTime ?? 0);
    
    /* updateMap now takes a callback to re-enable the button after image load */
